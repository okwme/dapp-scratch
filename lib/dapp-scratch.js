...
class DappScratch {
    ...
    */
    ...
    writeWrapper () {
      return new Promise((resolve, reject) => {
        if (fs.existsSync('./dapp-module')) fs.removeSync('./dapp-module')
        fs.mkdirSync('./dapp-module')
        ...
        try {
          this.abi.forEach((info) => {
            ...
            if (info.stateMutability === 'nonpayable') {
              all += '    if (!this.account) return Promise.reject(new Error(\'Unlock Wallet\'))\n'
            }

            all += '    return this.' + this.contractName_ + '.methods.' + info.name + '(' + typedFunctions + ')'
